...
const highlightCategory = [
  ...
  },
  {
    title: "Tênis",
    component: (
      <Sneakers className="fill-lightGray stroke-lightGray hover:stroke-primary hover:fill-primary" />
    ),
  },
];
...
const HighlightCollection = () => {
  const { allProducts, setFilteredProducts } = useContext(FilterContext);
  ...
  return (
    <section className="w-full py-[38px] h-full">
      ...
      <div className="w-full">
        <h3 className="text-large-bold text-center mb-10">
          Coleções em destaque
        </h3>
        <div className="w-full flex flex-row justify-center items-center gap-x-[68px]">
          {highlightCategory.map((collection, index) => (
            <div key={index} onClick={() => handleOnClick(collection.title)}>
              <div className="mb-3 w-24 flex justify-center cursor-pointer">
                {collection.component}
                <span className="sr-only">{collection.title}</span>
              </div>
              <h5 className="text-xsmall-bold text-center">
                {collection.title}
              </h5>
            </div>
          ))}
